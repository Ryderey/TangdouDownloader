...
echo ""
echo "[4/6] 安装依赖..."
uv pip install flask gunicorn requests beautifulsoup4 lxml redis rq
...
cat > "$HOME/.config/systemd/user/tangdou-worker.service" << EOF
[Unit]
Description=Tangdou MP3 Extractor Worker (User Service)
After=network.target redis.service
Wants=redis.service
...
ExecStart=$INSTALL_DIR/.venv/bin/python $INSTALL_DIR/scripts/rq_worker.py
Restart=always
RestartSec=5
...
cat > "$HOME/.config/systemd/user/tangdou-cleanup.service" << EOF
[Unit]
Description=Tangdou MP3 Cleanup
...
Environment="PATH=$INSTALL_DIR/.venv/bin:$HOME/.cargo/bin:$PATH"
Environment="PYTHONPATH=$INSTALL_DIR"
ExecStart=$INSTALL_DIR/.venv/bin/python $INSTALL_DIR/scripts/cleanup.py
EOF

cat > "$HOME/.config/systemd/user/tangdou-cleanup.timer" << EOF
[Unit]
Description=Run Tangdou MP3 cleanup every 30 minutes
...
echo "  Worker日志: journalctl --user -u tangdou-worker -f"
echo "  清理定时:  systemctl --user status tangdou-cleanup.timer"
echo ""
echo "🌐 访问地址:"
