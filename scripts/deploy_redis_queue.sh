...
systemctl --user stop tangdou-cleanup.timer 2>/dev/null || true
sleep 2
...
chmod +x "$PROJECT_DIR/scripts/cleanup.py"
...
# Cleanup定时器
cp "$PROJECT_DIR/scripts/tangdou-cleanup.service" "$HOME/.config/systemd/user/"
cp "$PROJECT_DIR/scripts/tangdou-cleanup.timer" "$HOME/.config/systemd/user/"
...
echo "  Cleanup: systemctl --user status tangdou-cleanup.timer"
echo ""
echo "查看日志:"
echo "  Web:   journalctl --user -u tangdou-mp3 -f"
echo "  Worker: journalctl --user -u tangdou-worker -f"
echo "  Cleanup: journalctl --user -u tangdou-cleanup.service -f"
echo ""
echo "队列状态:"
